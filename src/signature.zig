...
pub fn Signature(comptime variant: Variant) type {
    ...
    return extern struct {
        ...
        /// Multithreaded aggregate verify
        pub fn mtAggregateVerify(
            self: *const Self,
            memory_pool: *std.heap.MemoryPool([pairing_size]u8),
            ...
            var atomic_valid = std.atomic.Value(?BlstError).init(null);
            var wg = std.Thread.WaitGroup{};

            const pairing_ptr = try memory_pool.create();
            var acc = Pairing(variant).init(pairing_ptr, true, dst) catch {
                return BlstError.FAILED_PAIRING;
            };
                ...
                thread_pool.spawnWg(
                    ...
                    struct {
                        fn run(
                            ...
                            _pks_validate: bool,
                            _pool: *std.heap.MemoryPool([pairing_size]u8),
                            _atomic_counter: *std.atomic.Value(usize),
                            _atomic_valid: *std.atomic.Value(?BlstError),
                            _acc: *Pairing(variant),
                        ) void {
                            var pairing = Pairing(variant).init(_pool, true, _dst) catch {
                                // .release will publish the value to other threads
                                _atomic_valid.store(error.BLST_FAILED_PAIRING, .release);
                                return;
                            };
                    ...
                    .{ msgs, dst, pks, pks_validate, memory_pool, &atomic_counter, &atomic_valid, &acc },
                );
            }
            ...
            if (atomic_valid.load(.monotonic)) |e| {
                return e;
            }
        ...
        // pub fn mtVerifyMultipleAggregateSignatures(
        //     msgs: [][]const u8,
        //     dst: []const u8,
        //     pks: []const *PublicKey,
        //     pks_validate: bool,
        ...
        pub fn uncompress(sig_comp: []const u8) BlstError!Self {
            ...
            if (len == V.sig_compress_size and (sig_comp[0] & 0x80) != 0) {
                var sig = Self{};
                return V.sig_uncompress(&sig.point, &sig_comp[0]);
            }
